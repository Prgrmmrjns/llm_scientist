...
def generate_latex_report(studies, filepath="systematic_review.tex", intro_content=None):
    """
    Generate a LaTeX report from the systematic review results.
    
    Args:
        studies: List of StudyMetadata objects
        filepath: Output file path
        intro_content: Optional IntroductionContent object with generated content
    """
    # Create a new document
    doc = Document()
    
    # Add packages for table formatting and additional features
    doc.packages.append(NoEscape(r'\usepackage{array}'))
    doc.packages.append(NoEscape(r'\usepackage{tabularx}'))
    doc.packages.append(NoEscape(r'\usepackage{titling}'))  # For subtitle support
    
    # Add title
    if intro_content:
        title = escape_latex(intro_content.title)
        if intro_content.subtitle:
            # Combine title and subtitle
            subtitle = escape_latex(intro_content.subtitle)
            doc.preamble.append(NoEscape(r'\title{' + title + r'\\\large{' + subtitle + r'}}'))
        else:
    ...
    with doc.create(Section('Abstract')):
        if intro_content:
            doc.append(f'This systematic review examines {intro_content.title.lower()}. ')
            doc.append('We analyzed recent publications to identify trends, methodologies, and key findings in this domain. ')
            doc.append('Our analysis reveals important insights for researchers and practitioners.')
        else:
    ...
    with doc.create(Section('Introduction')):
        if intro_content:
            # Add key points
            doc.append('This systematic review addresses the following key aspects:')
            doc.append(NoEscape(r'\begin{itemize}'))
            for point in intro_content.key_points:
                doc.append(NoEscape(r'\item ' + escape_latex(point)))
            doc.append(NoEscape(r'\end{itemize}'))
            
            # Add spacing before research questions
            doc.append(NoEscape(r'\vspace{0.5cm}'))
            
            # Add research questions
            doc.append('The review aims to address the following research questions:')
            doc.append(NoEscape(r'\begin{enumerate}'))
            for question in intro_content.research_questions:
                doc.append(NoEscape(r'\item ' + escape_latex(question)))
    ...
    with doc.create(Section('Results')):
        ...
        with doc.create(Subsection('Overview')):
            ...
            years = [study.year for study in studies if study.year is not None]
            if years:
                doc.append(f'Publication years ranged from {min(years)} to {max(years)}. ')
            avg_relevance = sum(study.relevance_score or 0 for study in studies) / len(studies)
        ...
        with doc.create(Subsection('Study Characteristics')):
            with doc.create(Table(position='htbp')) as table:
                ...
                for study in studies:
                    ...
                    title = escape_latex(study.title)
                    journal = escape_latex(study.journal or 'N/A')
                    year = study.year or 'N/A'
                    score = study.relevance_score or 'N/A'
                    
                    # Handle missing abstracts
                    if study.abstract:
                        key_finding = study.abstract.split('.')[0].strip() + '.'
                    else:
...
def generate_plots(studies):
    ...
    plt.subplot(3, 1, 1)
    journals = [study.journal or 'Unknown' for study in studies]
    ...
    # 2. Studies per year
    plt.subplot(3, 1, 2)
    years = [study.year or 0 for study in studies]
    ...
    plt.subplot(3, 1, 3)
    scores = [study.relevance_score or 0 for study in studies]
